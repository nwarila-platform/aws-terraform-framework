...
resource "aws_instance" "us_east_1" {
  ...
  # Interfaces must be attached at creation. Otherwise EC2 adds a default interface regardless of
  # what else is configured, and attaching later requires the instance to be powered off.
  dynamic "network_interface" {
    for_each = local.instance_network_interfaces.us_east_1[each.key]
