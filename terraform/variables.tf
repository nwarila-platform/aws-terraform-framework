...
variable "all_systems" {
  ...
  validation {
    ...
    error_message = join(" ", [
      "tags.Backup, readiness_gate, root_block_device, ebs_block_devices, and",
      "ami_block_device_overrides carry no defaults and must not be null. Give each a real",
      "value; use [] for an empty ebs_block_devices or ami_block_device_overrides rather than",
      "null. Network-interface ingress and egress rule ip_protocol values must not be null.",
    ])
  }
  ...
  validation {
    condition = alltrue([
      for system in var.all_systems : length(system.network_interfaces) >= 1
    ])
    error_message = join(" ", [
  ...
  validation {
    condition = alltrue([
      for tags in concat(
        ...
        flatten([
          for system in var.all_systems : [
            for nic in system.network_interfaces : nic.tags == null ? {} : nic.tags
          ]
        ]),
        ) : alltrue([
...
variable "all_databases" {
  ...
  validation {
    condition = nonsensitive(length(distinct([
...
# restate any of these in a value file. All four are required and non-nullable - there is no
# "unattributed" deployment.
variable "repository" {
  description = <<-EOT
    Owner/name slug of the repository this deployment was applied from (e.g.
    nwarila-platform/aws-terraform-framework).
  EOT
  type        = string
  nullable    = false
  ...
  validation {
    ...
    error_message = join(" ", [
      "repository must be an owner/name slug (e.g.",
      "nwarila-platform/aws-terraform-framework).",
    ])
  }
}
...
variable "repository_id" {
  description = <<-EOT
    Numeric, rename-stable GitHub repository id. Survives a repository rename, which the slug does
    not.
  EOT
  type        = string
  nullable    = false
...
variable "commit_sha" {
  description = <<-EOT
    The checked-out commit this deployment was applied from: git rev-parse HEAD after checkout,
    NOT github.sha, which points at the merge commit for pull_request events.
  EOT
  type        = string
  nullable    = false
  ...
  validation {
    ...
    error_message = join(" ", [
      "commit_sha must be the lowercase 40-character checked-out commit SHA (git rev-parse HEAD",
      "after checkout, not github.sha).",
    ])
  }
}
...
variable "run_id" {
  description = <<-EOT
    Numeric GitHub Actions run id. The run record holds actors, timestamps, and approvals; those
    stay in deployment evidence rather than in tags.
  EOT
  type        = string
  nullable    = false
