...
- `terraform/tests/load_balancers.tftest.hcl` verifies that load balancers are
  ...
  subnet or subnet-mapping shape, plan with the expected load-balancer type, and
  retain target groups that intentionally match zero systems.
- `terraform/tests/shared_ebs_volumes.tftest.hcl` verifies shared-volume and
  attachment identities, cross-system ownership and device claims,
  Multi-Attach wiring, and the KMS alias suffix convention.
...
- `terraform/tests/runner_ingress.tftest.hcl` verifies run-scoped group and rule
  creation, runner/operator transport scoping, and exclusion of SSM-reached
  systems.
- `terraform/tests/security_group_rule_sources.tftest.hcl` verifies that an
  ingress or egress rule sets exactly one source.
- `terraform/tests/systems.tftest.hcl` verifies that EC2 instance-state
  ...
  that hyphenated and underscored east-region inputs normalize to the same
  regional bucket. It asserts module-owned encryption, volume-destroy, and
  IMDSv2 hardcodes. Private-address runs cover primary-address subnet and
  reserved-address checks, subnet-scoped uniqueness, additional-address
  ordering, both off spellings, entry shape, the pinned-primary requirement,
...
- `terraform/tests/managed.tftest.hcl` verifies that a system referencing only
  ...
  association.
- `terraform/tests/ami_selector_grammar.tftest.hcl` verifies catalog truncation,
  atomic build-date placement, lowercase selectors, the literal-ID bypass, and
  catalog-versus-literal owner selection.
...
- `terraform/tests/connection_type.tftest.hcl` verifies direct and SSM transport
  selection, Windows bootstrap and password-fetch behavior, the SSM gate
  prohibition, and rejection of WinRM on Linux.
...
- `terraform/tests/validation_coverage.tftest.hcl` provides 26 negative runs for
  repository metadata, explicit-null handling, and load-balancer variable
  validations.
- `make ci` also runs formatting, `terraform init`, validation, TFLint,
  ...
  bidirectional deny-all `.gitignore` allowlist guard. Source checks prove that
  instance-state changes follow the readiness gate and Elastic IP association
  waits for both EC2 instance families.

## What The Tests Do Not Cover

- They do not apply resources to a live AWS account.
- They do not prove the referenced AMIs, EBS snapshots, KMS aliases, key pairs,
  subnets, or security groups exist or are usable in a consumer account.
