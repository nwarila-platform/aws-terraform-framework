...
run "alias_with_vpc_id_drives_the_interface_security_group" {
  command = apply

  assert {
    condition     = length(data.aws_subnet.us_east_1_inline_security_group) == 1 && contains(keys(data.aws_subnet.us_east_1_inline_security_group), "poc-net")
    error_message = "An alias with vpc_id must retain a subnet lookup instance keyed by its raw symbol."
  }
...
run "alias_without_vpc_id_falls_back_to_the_subnet_lookup" {
  ...
  assert {
    condition = (
      length(data.aws_subnet.us_east_1_inline_security_group) == 1 &&
      contains(keys(data.aws_subnet.us_east_1_inline_security_group), "poc-net") &&
      data.aws_subnet.us_east_1_inline_security_group["poc-net"].id == "subnet-0123456789abcdef0"
    )
    error_message = "An alias without vpc_id must use its raw symbol as the instance key and its resolved subnet id as the lookup argument."
  }
...
run "managed_networks_and_network_aliases_resolve_together" {
  command = apply
    ...
    all_systems = [
      {
        ...
        imds_hop_limit       = 1
        set_state            = null
        tags                 = { Function = "Managed resolution", Backup = true }
        root_block_device = {
          delete_on_termination = true
          iops                  = null
          tags                  = {}
      ...
      },
      {
        ...
        imds_hop_limit       = 1
        set_state            = null
        tags                 = { Function = "Alias resolution", Backup = true }
        root_block_device = {
          delete_on_termination = true
          iops                  = null
          tags                  = {}
  ...
  assert {
    condition = (
      local.elastic_network_interfaces.us_east_1["managed-host-eni-0"].subnet_id == aws_subnet.us_east_1["managed-net"].id &&
      local.elastic_network_interfaces.us_east_1["alias-host-eni-0"].subnet_id == "subnet-0123456789abcdef0"
    )
    error_message = "Managed and alias subnet names must resolve through their respective paths when both maps are populated."
  }
}
...
run "rejects_a_key_shared_with_managed_networks" {
  command = plan
...
run "rejects_an_alias_vpc_id_that_names_a_managed_network" {
  command = plan
